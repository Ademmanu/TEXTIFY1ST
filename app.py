...
def send_hourly_owner_stats():
    ...
    if not rows:
        msg = "⏰ Hourly Report (based on UTC time): no splits in the last hour. 😴"
    ...
    for r in rows:
        ...
        lines.append(part)
    body = "⏰ Hourly Report (based on UTC time) — last 1h:\n" + "\n".join(lines)
    for oid in OWNER_IDS:
        try:
...
def handle_command(user_id: int, username: str, command: str, args: str):
    ...
    if command == "/stats":
        ...
        cutoff_wat = utc_to_wat_ts(cutoff_utc.strftime("%Y-%m-%d %H:%M:%S"))
        with _db_lock, sqlite3.connect(DB_PATH, timeout=30) as conn:
            c = conn.cursor()
        ...
        send_message(user_id, f"📊 Last 12 hours (since {cutoff_wat}): *{words}* words split.")
        return jsonify({"ok": True})
    ...
    if command == "/botinfo":
        ...
        for r in active_rows:
            ...
            lines_active.append(f"⚙️ {uid}{name} - {int(rem)} remaining - {int(ac)} active - {int(get_queued_for_user(uid))} queued")
        lines_stats = []
        for r in stats_rows:
            uid, uname, s = r
            name = f" ({uname})" if uname else ""
