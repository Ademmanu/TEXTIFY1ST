...
This file is a complete standalone word_splitter_bot.py.
"""
...
# Formatting helpers (usernames displayed without '@')
...
# Ensure ALLOWED_USERS from env auto-added as allowed at startup (preserve behavior)
...
# Efficient token bucket using Condition to avoid busy-wait loops
class TokenBucket:
...
# Helper: UTF-16 length (used to compute Telegram entity offsets)
...
def increment_failure(user_id: int):
    try:
        ...
        if user_id in OWNER_IDS:
            logger.warning("Send failure for owner %s: failures=%s", user_id, failures)
            return
        if failures >= 6:
...
def send_message(chat_id: int, text: str):
    """
    Send plain text. Numeric IDs inside text are tagged as code entities.
    Uses the TokenBucket to rate-limit outgoing requests precisely.
    """
    if not TELEGRAM_API:
...
def record_split_log(user_id: int, username: str, count: int = 1):
    """
    Insert a single row with words=count to avoid per-word inserts when helpful.
    But preserve compatibility with existing aggregation queries.
    """
    if count <= 0:
        return
    try:
        conn = get_db_conn()
        c = conn.cursor()
        c.execute("INSERT INTO split_logs (user_id, username, words, created_at) VALUES (?, ?, ?, ?)",
                  (user_id, username, int(count), now_ts()))
        conn.commit()
    except Exception:
    ...
    try:
        while not stop_event.is_set():
            ...
            while i < total and not stop_event.is_set():
                ...
                # record split log and update sent_count
                try:
                    with _db_lock:
                        conn = get_db_conn()
                        c = conn.cursor()
                        c.execute("INSERT INTO split_logs (user_id, username, words, created_at) VALUES (?, ?, ?, ?)",
                                  (user_id, fetch_display_username(user_id) or str(user_id), 1, now_ts()))
                        c.execute("UPDATE tasks SET sent_count = ? WHERE id = ?", (i + 1, task_id))
                        conn.commit()
                except Exception:
                    logger.exception("Failed to record split log or update sent_count for task %s", task_id)

                i += 1

                woke = wake_event.wait(timeout=interval)
                wake_event.clear()
                if woke:
                    continue
...
def webhook():
    ...
    try:
        if "message" in update:
            ...
            # Preserve original behavior: ensure an allowed_users row exists for this user (insert if missing), then update username
            try:
                with _db_lock:
                    conn = get_db_conn()
                    c = conn.cursor()
...
# Command handlers (kept intact from original behavior)
